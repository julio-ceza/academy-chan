  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <main className="max-w-6xl mx-auto p-6 pt-10">
        {/* Hero Section */}
        <div className="relative bg-gradient-to-br from-emerald-600 via-emerald-500 to-emerald-600 rounded-3xl shadow-2xl p-10 mb-8 overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xl text-emerald-50 mb-6">Gerencie suas solicitações de suporte de forma rápida e eficiente.</p>
            
            {/* Estatísticas Rápidas */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                <div className="text-3xl font-black text-white">{tickets.length}</div>
                <div className="text-sm text-emerald-50">Total de Chamados</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                <div className="text-3xl font-black text-white">{tickets.filter(t => t.status !== 'Resolvido').length}</div>
                <div className="text-sm text-emerald-50">Em Aberto</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                <div className="text-3xl font-black text-white">{tickets.filter(t => t.status === 'Resolvido').length}</div>
                <div className="text-sm text-emerald-50">Resolvidos</div>
              </div>
            </div>
            
            {/* BOTÃO NOVO CHAMADO */}
            <Link href="/new-ticket" className="inline-flex items-center justify-center gap-3 w-full bg-white hover:bg-emerald-50 text-emerald-600 font-bold py-4 px-6 rounded-xl transition-all transform hover:scale-105 text-center shadow-xl hover:shadow-2xl">
        ...
        <section className="bg-slate-800/70 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-slate-700/50">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              <h2 className="text-3xl font-black text-white mb-1">📊 Minhas Solicitações</h2>
              <p className="text-slate-400">Mostrando <span className="text-emerald-400 font-bold">{filteredTickets.length}</span> de <span className="text-emerald-400 font-bold">{tickets.length}</span> chamados</p>
            </div>
          ...
          {isLoading ? (
             <div className="flex flex-col items-center justify-center py-16">
               ...
               <p className="text-slate-400 font-medium">Carregando seus chamados...</p>
             </div>
          ) : (
                      ...
                      <td colSpan="5" className="p-16">
                        <div className="flex flex-col items-center justify-center text-center">
                          ...
                          <p className="text-xl font-bold text-slate-300 mb-2">
                            {searchTerm ? 'Nenhum resultado encontrado' : 'Nenhum chamado ainda'}
                          </p>
                          <p className="text-slate-400">
                            {searchTerm ? 'Tente buscar com outro termo' : 'Crie seu primeiro chamado acima'}
                          </p>
                        </div>
                      </td>
                    </tr>
